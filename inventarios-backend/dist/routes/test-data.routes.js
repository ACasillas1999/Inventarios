 ...
 */
function getAnyAvailablePool(connectionManager, preferredBranchId) {
    const statuses = connectionManager.getBranchesStatus();
    logger_1.logger.info(`Available branches: ${JSON.stringify(statuses)}`);
    let pool = connectionManager.getPool(preferredBranchId);
    if (pool)
        return { pool, branchId: preferredBranchId };
    // Buscar el primero que esté conectado
    const firstConnected = statuses.find(s => s.status === 'connected');
    if (firstConnected) {
        logger_1.logger.info(`Branch ${preferredBranchId} not found or error. Using branch ${firstConnected.id} instead.`);
...
router.get('/stats', auth_1.authMiddleware, requireAdmin, async (req, res) => {
    ...
    try {
        ...
        let totalArticles = 0;
        try {
            const [totalResult] = await branchPool.query('SELECT COUNT(*) as total FROM articulo');
            totalArticles = totalResult[0].total;
        }
        catch (e) {
            logger_1.logger.warn('Failed to query "articulo" table, trying "Articulos"...');
            const [totalResult] = await branchPool.query('SELECT COUNT(*) as total FROM Articulos');
            totalArticles = totalResult[0].total;
        }
        // 2. Contar artículos contados (Local)
        logger_1.logger.debug(`Querying counted articles from local database for branch ${branchId}...`);
        const [countedResult] = await localPool.query("SELECT COUNT(DISTINCT cd.item_code) as counted FROM count_details cd JOIN counts c ON cd.count_id = c.id WHERE c.branch_id = ?", [branchId]);
        const countedArticles = countedResult[0].counted;
        // 3. Contar registros de prueba (Local)
        logger_1.logger.debug(`Querying test records from local database for branch ${branchId}...`);
        const [testCountsResult] = await localPool.query("SELECT COUNT(*) as test_counts FROM counts WHERE folio LIKE 'TEST-COVERAGE-%' AND branch_id = ?", [branchId]);
        const testCounts = testCountsResult[0].test_counts;
        const [testDetailsResult] = await localPool.query("SELECT COUNT(*) as test_details FROM count_details WHERE count_id IN (SELECT id FROM counts WHERE folio LIKE 'TEST-COVERAGE-%' AND branch_id = ?)", [branchId]);
        ...
        const coveragePercentage = totalArticles > 0
            ...
            : '0.00';
        logger_1.logger.info('Stats fetched successfully');
        res.json({
            branch_id: branchId,
        ...
        logger_1.logger.error('Error getting test data stats:', error);
...
router.post('/seed-coverage', auth_1.authMiddleware, requireAdmin, async (req, res) => {
    logger_1.logger.info('POST /api/test-data/seed-coverage requested');
    try {
        const user = req.user;
        const localPool = (0, database_1.getLocalPool)();
        const connectionManager = ConnectionManager_1.ConnectionManager.getInstance();
        ...
        const [result1] = await localPool.query(`INSERT INTO counts (
            ...
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`, ['TEST-COVERAGE-001', branchId, 1, 'total', 'inventario', 'contado', 'media', user.id, user.id, 5]);
        const countId1 = result1.insertId;
        // Insertar 5000 artículos aleatorios desde el remoto al local
        logger_1.logger.debug(`Fetching items from remote branch ${branchId} to seed...`);
        ...
        let items = [];
        try {
            const [rows] = await branchPool.query(itemsQuery);
            items = rows;
        }
        catch (e) {
            logger_1.logger.warn('Failed to query "articulo" with "Unidad_Medida", trying "Unidad"...');
        ...
        logger_1.logger.debug(`Inserting ${items.length} items into local count_details...`);
        ...
        if (items.length > 0) {
            ...
            await localPool.query(`INSERT INTO count_details (
        ...
        const [result2] = await localPool.query(`INSERT INTO counts (
                folio, branch_id, almacen, type, classification, status, priority,
                responsible_user_id, created_by_user_id, tolerance_percentage,
                created_at, updated_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`, ['TEST-COVERAGE-002', branchId, 1, 'ciclico', 'inventario', 'contado', 'alta', user.id, user.id, 5]);
        ...
        let diffItems = [];
        try {
            const [rows] = await branchPool.query(diffItemsQuery, [spawnedItems.length > 0 ? spawnedItems : ['']]);
            diffItems = rows;
        }
        catch (e) {
        ...
        const [afterCounted] = await localPool.query("SELECT COUNT(DISTINCT item_code) as counted FROM count_details cd JOIN counts c ON cd.count_id = c.id WHERE c.branch_id = ?", [branchId]);
        ...
        logger_1.logger.error('Error seeding test data:', error);
...
router.delete('/cleanup-coverage', auth_1.authMiddleware, requireAdmin, async (req, res) => {
    ...
    try {
        ...
        const [countsCount] = await localPool.query("SELECT COUNT(*) as count FROM counts WHERE folio LIKE 'TEST-COVERAGE-%' AND branch_id = ?", [branchId]);
        ...
        // Eliminar conteos
        await localPool.query("DELETE FROM counts WHERE folio LIKE 'TEST-COVERAGE-%' AND branch_id = ?", [branchId]);
        // Obtener estadísticas después
        const [afterCounted] = await localPool.query("SELECT COUNT(DISTINCT item_code) as counted FROM count_details cd JOIN counts c ON cd.count_id = c.id WHERE c.branch_id = ?", [branchId]);
        const afterCount = afterCounted[0].counted;
        ...
        logger_1.logger.error('Error cleaning test data:', error);
