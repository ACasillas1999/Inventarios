...
class ReportsService {
     ...
     */
    async getBulkRequestsReport(filters) {
        const BULK_STATUSES = ['pendiente', 'en_revision', 'ajustado', 'rechazado'];
        const buildWhere = (alias, params) => {
        ...
        // --- Resumen por estatus ---
        const statusParams = [];
        const [statusRows] = await this.pool.execute(`SELECT status, COUNT(*) as total FROM bulk_requests${buildWhere('bulk_requests', statusParams)} GROUP BY status`, statusParams);
        const by_status = { pendiente: 0, en_revision: 0, ajustado: 0, rechazado: 0 };
        let total = 0;
        ...
        const [priorityRows] = await this.pool.execute(`SELECT priority, COUNT(*) as total FROM bulk_requests${buildWhere('bulk_requests', priorityParams)} GROUP BY priority`, priorityParams);
        const by_priority = priorityRows.map((row) => ({ priority: row.priority, total: Number(row.total) }));
        // --- Resumen por sucursal (pivot estatus) ---
        const branchParams = [];
        const [branchRows] = await this.pool.execute(`SELECT br.branch_id, b.name as branch_name, br.status, COUNT(*) as total
             FROM bulk_requests br
             JOIN branches b ON br.branch_id = b.id
             ${buildWhere('br', branchParams)}
             GROUP BY br.branch_id, b.name, br.status`, branchParams);
        ...
        const [resOverallRows] = await this.pool.execute(`SELECT AVG(TIMESTAMPDIFF(HOUR, created_at, reviewed_at)) as avg_hours, COUNT(*) as resolved_count
             FROM bulk_requests${resOverallWhere}`, resOverallParams);
        const overall_avg_hours = resOverallRows[0]?.avg_hours != null ? Number(resOverallRows[0].avg_hours) : null;
        const resBranchParams = [];
        const resBranchWhere = buildWhere('br', resBranchParams) +
            " AND br.status IN ('ajustado', 'rechazado') AND br.reviewed_at IS NOT NULL";
        const [resBranchRows] = await this.pool.execute(`SELECT br.branch_id, b.name as branch_name,
                    AVG(TIMESTAMPDIFF(HOUR, br.created_at, br.reviewed_at)) as avg_hours,
                    COUNT(*) as resolved_count
             FROM bulk_requests br
             JOIN branches b ON br.branch_id = b.id
             ${resBranchWhere}
             GROUP BY br.branch_id, b.name`, resBranchParams);
        ...
        const [resReviewerRows] = await this.pool.execute(`SELECT br.reviewed_by_user_id as user_id, u.name as user_name,
                    AVG(TIMESTAMPDIFF(HOUR, br.created_at, br.reviewed_at)) as avg_hours,
                    COUNT(*) as resolved_count
             FROM bulk_requests br
             ...
             ORDER BY avg_hours ASC`, resReviewerParams);
        ...
        // --- Ranking de usuarios ---
        const reqParams = [];
        const [requesterRows] = await this.pool.execute(`SELECT br.requested_by_user_id as user_id, u.name as user_name, COUNT(*) as total
             FROM bulk_requests br
        ...
        const [reviewerRows] = await this.pool.execute(`SELECT br.reviewed_by_user_id as user_id, u.name as user_name, COUNT(*) as total
             FROM bulk_requests br
             JOIN users u ON br.reviewed_by_user_id = u.id
             ${revWhere}
             GROUP BY br.reviewed_by_user_id, u.name
        ...
        const filesParams = [];
        const [fileRows] = await this.pool.execute(`SELECT f.id as file_id, f.original_name, br.folio, COUNT(d.id) as download_count
             ...
             ORDER BY download_count DESC`, filesParams);
